...
4. If you add a code path that writes `ACTIVE_OVERRIDE` outside `reapply_on_active_primary` / `take_active_override`, the "latest user DNS survives TUN" property breaks — audit that path against all four invariants.
