...
type FetchedMode = { mode: string; json: string };
...
function emitGeneratedFile(
    ...
    const entries = fetched
        .map((r) => `    '${r.mode}': ${JSON.stringify(r.json)}`)
        .join(',\n');
 ...
 * conf-template repo. Read by \`getBuiltInTemplate\` in
 * \`src/config/templates/index.ts\` when the runtime template cache is empty
 * (first launch, offline, etc).
 *
 * Clients that can reach the network pick up fresher templates via the SWR
 * prime hook in \`hooks/useSwr.ts\`, so this snapshot is the floor, not the
 * ceiling — its age matches the app binary's ship date.
 */
export const BUILT_IN_TEMPLATES: Record<configType, string> = {
${entries},
};
`;
}
    ...
    try {
        const results = await Promise.all([
            ...
            ...modes.map(async (mode): Promise<FetchedMode> => {
                ...
                return { mode, json: JSON.stringify(parsed) };
            }),
        ]);
