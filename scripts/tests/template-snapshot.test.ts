...
// Branch:  dev
// sing-box: v1.14.0-beta.10
export const BUILD_TIME_TEMPLATE_SOURCE = {
    branch: 'dev',
    singBoxVersion: 'v1.14.0-beta.10',
};`;

    it('reuses a snapshot only when its branch and sing-box version match', () => {
        expect(canReuseGeneratedTemplateSnapshot(currentSnapshot, 'dev', 'v1.14.0-beta.10')).toBe(true);
        expect(canReuseGeneratedTemplateSnapshot(currentSnapshot, 'stable', 'v1.14.0-beta.10')).toBe(false);
    ...
    it('rejects files without generated metadata', () => {
        expect(canReuseGeneratedTemplateSnapshot('export const template = {};', 'dev', 'v1.14.0-beta.10'))
            .toBe(false);
    });
});
